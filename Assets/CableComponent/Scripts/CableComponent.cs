...
public class CableComponent : MonoBehaviour
{
	...
	void RenderCable()
	{
		if (useSplineSmoothing)
		{
			...
			// Add each physical particle position as a knot in the spline
			for (int pointIdx = 0; pointIdx <= segments; pointIdx++) 
			{
				spline.Add(new BezierKnot(points[pointIdx].Position));
			}

			// Apply AutoSmooth to the knots to create a clean, curved rope
			var allKnots = new SplineRange(0, spline.Count);
			spline.SetTangentMode(allKnots, TangentMode.AutoSmooth);

			// Render the spline curve at a higher resolution
			line.positionCount = splineResolution;
			for (int i = 0; i < splineResolution; i++)
			{
				float t = i / (float)(splineResolution - 1);
				Vector3 worldPos = (Vector3)SplineUtility.EvaluatePosition(spline, t);
				line.SetPosition(i, worldPos);
			}
		}
		else
		{
			// Direct rendering of particles without spline smoothing
